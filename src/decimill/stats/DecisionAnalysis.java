...
public class DecisionAnalysis {
    ...
    public static double evppi(double[] param, double[][] nbs) {
        ...
        if (nbs[0].length != n) {
            throw new StatsException("Input parameter and net benefits matrix must have the same dimension.");
        }
        ...
        for (int i = 0; i < d - 1; i++) {
            ...
            for (int j = i + 1; j < d; j++) {
                ...
                double[] cm = VU.div(VU.cumsum(VU.diff(nbs[i], nbs[j])), n);

                if (nSegs[i][j] == 1) {

                    int l = VU.whichMin(cm);
                    int u = VU.whichMax(cm);
        ...
        if (segPoints.length > 0) {
            ...
            int[] segPoints2 = VU.unique(newSegPoints);
            ...
            double[] means = VU.rowMeans(nbs);
            double max = VU.max(means);
